...
class App extends Component {
    state = {
        textCharacters: "Hello"
    }

    onTextChange(event) {
        this.setState({textCharacters: event.target.value});
    }

    onClickCharacter(position) {
        let partOne = this.state.textCharacters.substr(0, position)
        let partTwo = this.state.textCharacters.substr(position + 1)
        this.setState({textCharacters: partOne + partTwo})
    }
    ...
    render() {
        return (
            <div className="App">
                ...
                <p></p>
                <Textbox label="My name" value={this.state.textCharacters}
                         change={this.onTextChange.bind(this)}></Textbox>
                <ValidationComponent fieldName="My name" fieldValue={this.state.textCharacters}></ValidationComponent>
                <CharContainer fieldValue={this.state.textCharacters} charClick={this.onClickCharacter.bind(this)}></CharContainer>
            </div>
        );
    }
}
